...
import { blogEdit } from "@/services/blogServices";
...
const BlogEdit = () => {
  ...
  const { data: blogData, loading } = useFetch(
    `${import.meta.env.VITE_API_URL}/blog/show/${blogId}`,
    {
      method: "GET",
      credentials: "include",
    },
  ...
  async function onSubmit(values) {
    try {
      ...
      const response = await blogEdit(blogId, formData);
      if (response?.success) {
  ...
  useEffect(() => {
    if (blogData && blogData?.success) {
      ...
      setAvatarPreview(blogData?.blog?.featureImage);
    }
  }, [blogData]);
    ...
    <section className="flex items-center justify-center w-full min-h-full p-5">
      <Card className="w-full py-5 px-2">
        <CardHeader className="mb-2 flex flex-col items-center justify-center">
          <CardTitle className="font-bold text-2xl mb-1">Edit Blog</CardTitle>
          <CardDescription className="text-sm">
            Update your existing blog post details and content
          </CardDescription>
        </CardHeader>
        <CardContent>
